...
max_res.append(('Round', 'Loss', 'AUC', 'Precision', 'Recall', 'F1score'))
...
def create_model(input_a_shape, input_b_shape, weight=1e-3):
    ...
    # AlexNet-like model
    x = Conv1D(64, kernel_size=11, strides=4, padding='same', activation='relu')(concat)
    x = MaxPooling1D(pool_size=3, strides=2)(x)
    x = Conv1D(192, kernel_size=5, padding='same', activation='relu')(x)
    x = MaxPooling1D(pool_size=3, strides=2)(x)
    x = Conv1D(384, kernel_size=3, padding='same', activation='relu')(x)
    x = Conv1D(384, kernel_size=3, padding='same', activation='relu')(x)
    x = Conv1D(256, kernel_size=3, padding='same', activation='relu')(x)
#     x = MaxPooling1D(pool_size=3, strides=2)(x)
#     x = MaxPooling1D(pool_size=1, strides=1)(x)  # Apply pooling along the time dimension
...
#     print(x.shape)
    ...
    excitation = Dense(128, activation='relu')(x)
...
def get_evaluate_fn(model):
    ...
    def evaluate(
        ...
        if(f1 > save_f1):
            ...
            max_res.append((cnt, loss.result().numpy(), auc.result().numpy(), precision.result().numpy(), recall.result().numpy(), f1))
